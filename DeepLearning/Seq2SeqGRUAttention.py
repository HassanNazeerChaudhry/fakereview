...
from keras.layers import Input, GRU, Embedding, Dense, Attention

class Seq2SeqGRUAttention:
    ...
    def build_model(self):
        ...
        encoder_gru = GRU(self.gru_units, return_sequences=True, return_state=True)(encoder_embedding)

        # Decoder
        decoder_input = Input(shape=(None,))
        decoder_embedding = Embedding(input_dim=self.vocab_size, output_dim=self.embedding_dim)(decoder_input)
        decoder_gru = GRU(self.gru_units, return_sequences=True, return_state=True)(decoder_embedding, initial_state=encoder_gru[1])
        attention = Attention()([decoder_gru, encoder_gru[0]])
        decoder_concat = tf.concat([decoder_gru, attention], axis=-1)
